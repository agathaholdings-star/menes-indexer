...
import { Shield, Trash2, Eye, Ban, MessageCircle, Star, Users, ChevronDown } from "lucide-react";
...
// 管理者メールリスト（本番では環境変数 or DBで管理）
const ADMIN_EMAILS = ["info@agatha-holdings.co.jp", "reviewer@test.com"];
...
export default function AdminPage() {
  ...
  const [stats, setStats] = useState({ shops: 0, therapists: 0, reviews: 0, users: 0 });
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      // 統計
      const [shopCount, therapistCount, reviewCount, userCount] = await Promise.all([
      ...
      const { data: reviewData } = await supabase
        .from("reviews")
        .select("id, score, looks_type, body_type, service_level, comment_first_impression, created_at, user_id, therapist_id")
  ...
  const handleDelete = async () => {
    ...
    if (type === "review") {
      ...
      await supabase.from("bbs_posts").delete().eq("thread_id", id);
      await supabase.from("bbs_threads").delete().eq("id", id);
