...
export async function GET(req: NextRequest) {
  ...
  switch (tab) {
    case "stats": {
      // Only fetch counts — lightweight, always needed
      const [pendingCount, newContactCount] = await Promise.all([
        supabaseAdmin
          .from("reviews")
    ...
    case "reviews": {
      const { data: reviews } = await supabaseAdmin
        .from("reviews")
        .select(
          "id, score, looks_type_id, body_type_id, service_level_id, moderation_status, comment_reason, comment_first_impression, comment_style, comment_service, comment_service_detail, comment_cost, comment_revisit, comment_advice, created_at, user_id, therapist_id, salon_id, verification_image_path, is_verified"
        )
