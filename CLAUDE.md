...
- **セラピスト名修復**: v1完了（10,299/10,795件）、v2実行中（サロン名部分一致5,426件追加対象）。HTMLキャッシュ`html_cache/`保存済み
...
- **次のステップ**:
    ...
    10b. **セラピスト名修復** ← 🔄 v2実行中（2026-02-18）
        - 目的: Phase②スクレイピングで名前がPROFILE/サロン名になった件を修復。フロント表示の品質確保
        - `repair_therapist_names.py`: source_urlからHTML再取得→CSSセレクタ/H1/og:title→LLMフォールバック→DB UPDATE
        - v1完了: 10,299/10,795件修復（完全一致+PROFILE系）、成功率95.4%
        - v2実行中: サロン名**部分一致**（「目黒エブリシング」含む「エブリシング」等）5,426件追加対象
          - EXCLUDE_KEYWORDSに twitter/メンズエステ等を追加（v1で誤抽出した「twitterツイッター」440件も修復中）
          - レジューム: `python3 database/therapist-scraper/repair_therapist_names.py --resume`
        - HTMLはgzip圧縮で`html_cache/`に保存済み（再処理可能）
        - 一部キャッチコピー混入あり → ユーザーが目視チェックで対応予定
...
| therapists | 79,639 | Phase②完走。VPS→ローカル同期済み（2026-02-16） |
