...
- **スクレイパー強化v2: URL補完+ページネーション+フィルタ+dedup改善（2026-02-22）**:
    ...
    - **batch3テスト（新規10件、#102-#119）**: 7/10成功、100名登録
        - アロマエルメス46名、トレンディスパ20名、楽園十八式11名、ハッピー10名、ラベンダー9名、ブルームーン4名
        - 失敗3件: 年齢確認ゲート(藍花)、JS描画で名前なし(.click系2件: プルヌス、シーゼットガールズ/日がのぼる)
    ...
    - **再構築実行プラン**:
        ...
        3. VPS: `--full` を全6,489サロンで実行（3並列tmux、~30時間、~$225）
...
- **セラピスト統一スクレイピングパイプライン実装完了（2026-02-22）**:
    - **背景と課題**: 2スクリプト体制（`batch_extract_therapist_info.py` + `scrape_failed_salons.py`）で運用が煩雑。`scrape_failed_salons.py`にdedup がなく複数回実行で重複INSERT。`--new`モード（現`--full`）はtherapist_list_urlありサロンしか処理できなかった
    ...
    - **Phase 2: --fullモード（旧--new）に3段階Haikuフロー統合**:
        - `run_full()`（旧`run_new()`）を2パス構成に拡張（※後に1パスに簡素化）:
...
- **セラピスト情報Haiku一括抽出 設計・テスト完了（2026-02-19）**:
    ...
    - **本番実行スクリプト（✅ 実装完了 2026-02-19）**:
        - `batch_extract_therapist_info.py`: 全サロン3段階Haikuフロー
...
- **次のステップ**:
    ...
    12c. **スクレイピング課題＋インフラ懸念の洗い出し** → 📝 記録済み・検討中（2026-02-20）
        ...
        - 定期バッチへの画像DL統合（🔴→🟢対応中）: `--full`モードで自動統合（12d参照）
    ...
    12e-2. **Phase④ 300サロンテスト実行（2026-02-23、VPS 3並列）**:
        - **テスト構成**: 300サロン / 3ワーカー / 各100サロン
            - W1: --start-id 1 --end-id 101
            - W2: --start-id 101 --end-id 209
            - W3: --start-id 209 --end-id 337
        - **VPS掃除実施済み**: pg_dump バックアップ(61MB) → TRUNCATE therapists CASCADE + salon_scrape_cache + scrape_log → html_cache/ + checkpoint全削除
        - **結果**:
        ...
        - **データ品質**:
            ...
            - cupあり: 8,509（69%）
            - 紹介文あり: 11,356（92%）
            - 1サロン平均41名（旧スクレイパー平均15名から大幅増）
            - 279サロンにセラピスト紐付き（21店はセラピスト情報なし=正常）
        - **メモリ**: 3並列で792MB/1961MB使用（余裕あり）→ **本番は5並列可能**
        - **全6,489サロン換算**: 約13万名見込み（推定通り）
        - **ローカル同期済み**: pg_dump → ローカルSupabase投入（12,293名）→ 目視確認フェーズ
        - **次ステップ**: 目視確認OK後 → 本番5並列で全6,489サロン実行
...
- **方式**: `extract_therapist_info()`に画像抽出を統合したため、`--full`モードで新規セラピスト発見時に自動的にimage_urlsが取れる
- **未対応**:
