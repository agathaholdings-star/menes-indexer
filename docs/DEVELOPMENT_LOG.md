...
## 2026-03-04: セラピストDB重複クリーンアップ（228,816→128,636名）

### 発見: 2つの世界が混在していた

DBを調査した結果、セラピストテーブルに**2回分のスクレイピング結果が混在**していることが判明。

| バッチ | ID範囲 | 件数 | 時期 | 内容 |
|--------|--------|------|------|------|
| バッチ1（世界A） | 1〜99,999 | 93,894 | 2/11〜2/18 | Phase II ヒューリスティック + Phase III Haiku補強 |
| バッチ2（世界B） | 100,000〜244,520 | 134,922 | 2/18〜2/25 | Phase IV 統合パイプライン全店再スクレイピング |

重複3,437サロンで同じセラピストが2レコードずつ存在。例:
- salon 1（コス×コス）: 「あすか」がid=19とid=101508で2件（同一source_url、同一age/height）

### 根本原因: slug=NULLのON CONFLICT設計バグ

```python
# batch_extract_therapist_info.py
INSERT INTO therapists (...) VALUES (...)
ON CONFLICT (salon_id, slug) DO NOTHING  # ← slugがNULLなので絶対発火しない
RETURNING id

# INSERT後にslugを設定
UPDATE therapists SET slug = str(id) WHERE id = id  # ← 毎回違うslug
```

- PostgreSQLの`NULL != NULL`仕様で、`ON CONFLICT(salon_id, slug)`はslug=NULL同士を重複と判定しない
- slugをINSERT後にauto increment IDから設定するため、毎回異なるslugになる
- アプリ側の3点デデュプ（salon_id + source_url + name）は、VPS DBリセット後の再スクレイピングで空DBに対して実行されたため機能しなかった

### 判断: 世界Bのみに統一

- バッチ2（Phase IV）は統合パイプライン1本で全店処理 → データが均一
- バッチ1は複数プログラムのツギハギ（ヒューリスティック+LLM混在）
- 品質指標はバッチ1がわずかに上（プロフィール率93.5% vs 90.1%）だが、均一性を優先
- 口コミの`rewritten_reviews.json`は既にバッチ2のIDで紐づいていた

### 実施手順

1. 口コミ13,441件の`therapist_id`をバッチ1→バッチ2のIDに付け替え（salon_id + nameでマッチ）
2. バッチ1のうち口コミ紐づき以外の79,015件を削除
3. バッチ2内の重複6,286件を削除（salon_id + nameでグルーピング、最小ID残し）
4. 残りバッチ1（14,879件）も全削除 → 世界Bのみに統一
5. `step3_insert_reviews.py`再実行 → 11,446件投入

### 結果

| 項目 | Before | After |
|------|--------|-------|
| セラピスト | 228,816 | **128,636** |
| seed口コミ | 14,885 | **11,449**（うちseed 11,446） |
| データ均一性 | 2パイプライン混在 | Phase IV統合パイプラインのみ |

### 残課題（世界B内の品質）
- 一覧URL（/cast等）のまま: 34,940件
- プロフィール文なし: 18,028件
- 年齢なし: 19,198件
- 画像なし: 2,954件
- step3の1セラピスト1口コミ制約: rewritten_reviewsに16,462件あるが、5,000件がスキップ

## 2026-03-04: UXポリッシュ（feature/ux-polish）

- 修正0304.pdf 8件対応:
  1. サイドバーはみ出し修正（min-w-0追加）
  2. レビューモザイク確認（blur実装済み）
  3. セラピスト名・年齢表示修正（ReviewListにprops追加）
  4. スコアバッジ削除 + source_url表示追加
  5. ランキングnav削除（ページは残す）
  6. 競合表記「MEと同等」4箇所削除
  7. サロンパンくず修正（エリア情報props化）
  8. shadcn/ui確認（59コンポーネント）
- レビューカード全面改修:
  - 全件ブロック（index>0→全件isLocked）
  - スコア非表示（ロック時）
  - blur拡大（6セクション偽コンテンツ）
  - CTA文言「あなたの体験が、誰かの「ハズレ回避」になります」
- 表示ガード追加:
  - age=0非表示、height=0非表示
  - 画像なしフォールバック
  - セラピスト0件・口コミ0件メッセージ

---

## 2026-02-24: コンテキスト監査・メモリリストラクチャ実施

- **CLAUDE.md**: 232行→約110行にスリム化（スコア55→85見込み）
- **`.claude/rules/work-rules.md`** 新規作成: 作業ルールを切り出し
- **`database/therapist-scraper/CLAUDE.md`** 新規作成: VPS詳細・分類マスタ・ME口コミ移行・テストコマンドを集約
- **`docs/SYSTEM_DESIGN.md`** 更新: `shops`→`salons`修正、現行マイグレーションに基づくスキーマに更新
- **`docs/archive/`** に陳腐化2件を移動: AI_CONCIERGE_PLAN.md（Gemini計画）、SCRAPER_REFACTOR_NOTES.md（DEVELOPMENT_LOG.mdで代替済み）
- **Phase④ DB最新**: 104,585名（W1/W5稼働中、W2-W7完了）

## 2026-02-24: Phase④ VPS全件実行 進捗・クラッシュ記録

- **開始**: 2026-02-23 15:21 JST、5並列（W1-W5）
- **ワーカー構成（最終）**:
    | Worker | ID範囲 | 状態 | 備考 |
    |--------|--------|------|------|
    | W1 | 1-1538 | 稼働中 (888/1249) | |
    | W2 | 1538-3075 | `--resume`で復帰 (1/255) | 2回目クラッシュ |
    | W3 | 3075-4612 | ほぼ完了 (1373/1375) | |
    | W4 | 4612-5125 | 完了 (475/475) | 1回目クラッシュ後に範囲縮小して再起動 |
    | W5 | 6149-7686 | 稼働中 (954/1384) | |
    | W6 | 5125-5637 | 完了 (457/458) | W4クラッシュ後に新規追加 |
    | W7 | 5637-6149 | 完了 (492/492) | W4クラッシュ後に新規追加 |
- **DB登録数**: 85,026名（2026-02-24 09:30 JST時点）
- **クラッシュ2回発生**:
    | 回 | Worker | エラー | 対処 |
    |----|--------|--------|------|
    | 1回目 | W4 (4612-6149) | `AttributeError: 'NoneType' object has no attribute 'get'` | W4を4612-5125で再起動、残りをW6(5125-5637)+W7(5637-6149)に分割 |
    | 2回目 | W2 (1538-3075) | 同上（893/1055で停止） | `--resume`で再起動（残り162サロン→255件として処理中） |
- **メモリ**: 918MB/1,961MB使用（余裕あり）
- **完了見込み**: W1/W2/W5が残り。数時間で全完了予定
