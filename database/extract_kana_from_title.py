...
ルール:
1. 英語・アルファベット → 必ずカタカナに変換する（DAHLIA→ダリア、CLUB→クラブ、SPA→スパ、Lino→リノ、Belle→ベル、Aroma→アロマ）
2. 漢字・ひらがな → そのまま残す（天界のスパ→天界のスパ）
3. エリア名（恵比寿等）と「メンズエステ」等の一般語は除外
4. titleと参考が明らかに別の店 → ???
5. 出力は店名のみ。英語を含めない。説明禁止
