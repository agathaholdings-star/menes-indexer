...
Usage:
  ...
  python database/seed_reviews/step2_batch.py submit --delta           # 差分のみ投入（既存リライト済みをスキップ）
  ...
  python database/seed_reviews/step2_batch.py download --merge         # 結果取得＋既存リライト済みとマージ
"""
...
# submit
# ---------------------------------------------------------------------------
def _load_existing_rewritten_ids() -> set[int]:
    """既存rewritten_reviews.jsonからtherapist_idのセットを返す"""
    if not os.path.exists(OUTPUT_PATH):
        return set()
    with open(OUTPUT_PATH, "r", encoding="utf-8") as f:
        data = json.load(f)
    return {r["indexer_therapist_id"] for r in data.get("reviews", [])}
...
def cmd_submit(args):
    ...
    existing_ids = set()
    if args.delta:
        existing_ids = _load_existing_rewritten_ids()
        before = len(matches)
        matches = [m for m in matches if m["indexer_therapist_id"] not in existing_ids]
        print(f"  既存リライト済み: {len(existing_ids):,}件 → {before - len(matches):,}件スキップ")

    # 危険なURL（ドメインのみ等）のtherapist_idを除外
    EXCLUDE_IDS = {142046}  # https://miu.rapin-osaka.com（パスなし）
    matches = [m for m in matches if m["indexer_therapist_id"] not in EXCLUDE_IDS]
...
def cmd_download(args):
    ...
    if args.merge and os.path.exists(OUTPUT_PATH):
        ...
        # 新マッチのtherapist_idセット（有効な対象のみ残す）
        with open(RAW_PATH, "r", encoding="utf-8") as f:
            raw = json.load(f)
        valid_ids = {m["indexer_therapist_id"] for m in raw["matches"]}
        EXCLUDE_IDS = {142046}
        valid_ids -= EXCLUDE_IDS

        # 新規バッチ結果のtherapist_idセット
        new_ids = {r["indexer_therapist_id"] for r in results}

        # 旧から: 新マッチに含まれる & 新バッチで再生成されてないもの
        reused = [r for r in old_reviews if r["indexer_therapist_id"] in valid_ids and r["indexer_therapist_id"] not in new_ids]
        merged = reused + results

        print(f"\n  マージ: 旧から再利用 {len(reused):,}件 + 新規 {len(results):,}件 = {len(merged):,}件")
        results = merged
...
def main():
    ...
    p_submit.add_argument("--delta", action="store_true", help="差分のみ投入（既存リライト済みをスキップ）")

    # status
    sub.add_parser("status", help="バッチ状態確認")

    # download
    p_download = sub.add_parser("download", help="結果取得→rewritten_reviews.json")
    p_download.add_argument("--merge", action="store_true", help="既存リライト済みとマージ")
